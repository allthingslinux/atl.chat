...
Atheme uses a single comprehensive log file that captures all log levels:

```c
logfile "logs/atheme.log" { debug; };
```

The log file is written to `data/atheme/logs/atheme.log` on the host (mounted as `/usr/local/atheme/logs/` in the container). The `debug` level captures everything — admin actions, errors, info, network events, wallops, and registrations.

> **Note:** The `ATHEME_LOG_LEVEL` variable is defined in `.env.example` but is not actually consumed by the config template. The log level is hardcoded to `debug` in the template. To change the log level, you need to edit the `logfile` directive in `atheme.conf.template` directly.
