...
| Variable | Description | Required | Default |
|---|---|---|---|
| `OBSIDIANIRC_PORT` | Port for ObsidianIRC web IRC client | Yes | `8090` |
| `OBSIDIANIRC_IRC_WS_URL` | WebSocket URL for IRC connection | Yes | `wss://irc.atl.chat/ws` |
| `OBSIDIANIRC_AUTOJOIN` | Comma-separated list of channels to auto-join | No | `#general` |

ObsidianIRC is built with these values baked into the image as Vite build args. To change them, update `.env` and rebuild: `just obsidianirc rebuild`.
...
| `PROSODY_REST_TOKEN` | Bearer token for mod_http_admin_api (Portal uses this; generate with `just prosody-token`) | No | — |
| `PROSODY_REST_USERNAME` | Prosody REST admin username — *legacy, HTTP Basic; prefer PROSODY_REST_TOKEN* | No | `admin@atl.chat` |
| `PROSODY_REST_PASSWORD` | Prosody REST admin password — *legacy, HTTP Basic; prefer PROSODY_REST_TOKEN* | No | `change_me_prosody_rest_password` |
...
> **Warning:** `IRC_UNREAL_RPC_PASSWORD`, `PROSODY_REST_TOKEN` (or `PROSODY_REST_PASSWORD` if using legacy Basic auth), and `IRC_UNREAL_RPC_USER` are administrative credentials. If you deploy the Portal alongside this stack, ensure these match the corresponding service configuration and are changed from defaults. Portal expects `PROSODY_REST_TOKEN` (Bearer) for Prosody — use `just prosody-token` to generate.
