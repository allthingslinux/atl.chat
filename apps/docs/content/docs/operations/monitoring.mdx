...
Inside containers, logs are also written to files mounted under `data/`:

| Service | Container log path | Host bind-mount |
|---|---|---|
| UnrealIRCd | `/home/unrealircd/unrealircd/logs/` | `data/irc/logs/` |
| Atheme | `/usr/local/atheme/logs/` | `data/atheme/logs/` |
| Prosody | `/var/lib/prosody/logs/prosody.log` | `data/xmpp/logs/` |
| Bridge | stdout/stderr only | _(use `docker compose logs`)_ |
| The Lounge | `/var/opt/thelounge/` | `data/thelounge/` |
| WebPanel | nginx access/error logs | _(use `docker compose logs`)_ |
